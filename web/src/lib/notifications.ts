...
export const SMS_TEMPLATES = {
  // SMS Consent Confirmation (Twilio Compliance)
  OPT_IN_CONFIRMATION: () =>
    `HandyPro FL: You are subscribed to receive account and service-related text messages. Message frequency varies. Msg & data rates may apply. Reply STOP to opt out, HELP for help.`,
...
async function getUserContact(
  supabase: SupabaseClient,
  userId: string
): Promise<{ email: string | null; phone: string | null; first_name: string | null; sms_consent_given: boolean }> {
  const { data } = await supabase
    .from("profiles")
    .select("email, phone, first_name, sms_consent_given")
    .eq("id", userId)
    .single();
...
export async function notifyUser(
  ...
  try {
    ...
    // Send SMS (only if user has given explicit consent)
    if (prefs.sms_updates && contact.phone && contact.sms_consent_given && twilioConfigured) {
      try {
...
/**
 * Send SMS opt-in confirmation (Twilio compliance requirement)
 * Call this immediately after user gives SMS consent
 */
export async function sendSmsOptInConfirmation(
  ...
  try {
    ...
    if (!contact.sms_consent_given) {
      result.error = "User has not given SMS consent";
      return result;
    }
    ...
    try {
      await sendSms(contact.phone, SMS_TEMPLATES.OPT_IN_CONFIRMATION());
      result.sent = true;
      await logNotification(supabase, userId, "sms", "OPT_IN_CONFIRMATION", "sent");
    } catch (err) {
      result.error = err instanceof Error ? err.message : "SMS failed";
      await logNotification(supabase, userId, "sms", "OPT_IN_CONFIRMATION", "failed", result.error);
    }
  } catch (err) {
