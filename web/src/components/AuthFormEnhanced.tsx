...
export default function AuthFormEnhanced() {
  ...
  const sendOtp = async () => {
    ...
    if (!smsConsent) {
      setError('Please agree to receive SMS messages to use phone login.');
      return;
    }
    setLoading(true);
    setError(null);
  ...
  const verifyOtp = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (authError) {
      ...
      // Save SMS consent to profile and send opt-in confirmation
      if (data.user && smsConsent) {
        ...
        // Send opt-in confirmation SMS (Twilio compliance)
        try {
