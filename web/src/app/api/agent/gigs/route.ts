...
export async function GET() {
  ...
  const { data: catalog } = await adminSupabase
    ...
    .select("id, price_cents, base_minutes, general_skill, category");

  const catalogMap = new Map(catalog?.map((s) => [s.id, s]) || []);
  const normalizeSkillKey = (serviceId: string | null | undefined) => {
    if (!serviceId) return null;
    const meta = catalogMap.get(serviceId);
    const base =
  ...
  const gigs = (requests || [])
    ...
    .filter((gig) => {
      ...
      // Direct match on specific service
      if (agentSkills.includes(gig.service_type)) return true;

      // Fallback: match by general skill/category (e.g., assembly_bed -> assembly)
      const gigKey = normalizeSkillKey(gig.service_type);
      return gigKey ? agentSkillKeys.has(gigKey) : false;
    })
